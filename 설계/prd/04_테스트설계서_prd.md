...
> ⚠️ **myapp 적용 현황 (2026-08-29)** — myapp은 PostgreSQL·Redis를 쓰지 않습니다(`skipPaasData=true`). 아래 P-08·P-09·P-10(DB 공용 액세스·HA·백업)과 P-22(PostgreSQL Entra 인증)는 myapp 기준으로 **N/A**입니다(공통 샘플 앱처럼 실제 PostgreSQL을 쓰는 배포에서만 유효). 실제 `80_verify.ps1` 실행 결과(`배포/prd/reports/prd_verify_20260829-025219.json`)는 28개 중 21 Pass·7 Fail이며, Fail 7건은 전부 이 N/A 항목들입니다. myapp이 실제로 쓰는 P-01~07·P-11~21·P-23은 전부 Pass했습니다.
...
| TC 그룹 | 실행 | 근거 |
| --- | :---: | --- |
| 단위 U-A-01~08 · U-B-01~10 | ✅ 전체 | 외부 의존 없음 — 부작용 없음 |
| **단위 U-A-09~11 (비밀 출처 선택)** | ✅ | 순수 함수 — 비밀 미노출까지 검증 |
| 통합 I-A-01 `/healthz` | ✅ | 읽기 |
| 통합 I-A-02 `/readyz` | ✅ | 읽기 (**SQLite 연결까지 확인**) |
| 통합 I-A-03 분석 실행 | **⏭ 건너뜀** | **GitHub 실제 호출 + SQLite 쓰기 — 운영 데이터 오염 금지** |
| 통합 I-A-04 입력 검증(쓰기 없음) | ✅ | 저장되지 않으므로 안전 |
| 통합 I-A-05 이력 목록 조회 | ✅ | 읽기 |
| **통합 I-A-06 Notion 업로드** | **⏭ 건너뜀** | **실제 Notion 워크스페이스에 부수 효과 — 운영 금지** |
| 통합 I-A-07 이력 상세 조회 | ✅ | 읽기 |
| **통합 I-A-08 이력 삭제** | **⏭ 건너뜀** | 파괴적 쓰기 |
| **통합 I-A-09 감사로그 기록 확인** | **⏭ 건너뜀** | 분석 실행(쓰기)에 의존 |
| 통합 I-A-10 감사로그 접근 통제(403 확인) | ✅ | 읽기 전용 — 저장하지 않음 |
| **통합 I-A-11 `/version` 비밀 출처 노출** | ✅ **강화** | **Key Vault CSI 동기화까지 확인** · 비밀 문자열 없음 |
| E2E E-A-01 초기 화면 · E-A-02 입력 · E-A-05 이력 목록 | ✅ | 읽기 |
| **E2E E-A-03 분석 결과 · E-A-04 예외 · E-A-06·07 Notion 업로드** | **⏭ 건너뜀** | 쓰기·외부 SaaS 부수 효과 |
| E2E E-B-01~04 (테트리스) | ✅ | 서버 상태 없음 — 안전 |
| **운영 준비 검증 P-01~15** | ✅ | **이 환경의 핵심** |
| **아이덴티티 검증 P-16·17·18·19·20·21·23** | ✅ | **GitHub/Notion 비밀 없는 접근이 실제로 작동하는지**(myapp 은 PostgreSQL 을 쓰지 않으므로 P-22 는 공통 샘플 앱 전용 — [공통 06 §9](../공통/06_테스트설계서.md)) |
...
| **P-08** | **DB 공용 네트워크 액세스 차단** | `publicNetworkAccess = Disabled` | **인터넷에서 DB 접근 가능**(myapp은 **N/A** — PostgreSQL 미사용) |
| **P-09** | **DB 고가용성** | `highAvailability.mode ≠ Disabled` | 영역 장애 시 DB 중단(myapp은 **N/A**) |
| **P-10** | **DB 백업 보존** | ≥ 7일 (설정값 **14일**) | 논리적 손상 복구 불가(myapp은 **N/A**) |
...
| **P-22** | **PostgreSQL Entra 인증 + 관리 ID 가 Entra 관리자** | `activeDirectoryAuth = Enabled` **AND** `ad-admin` 에 principalId 존재 | 토큰 접속 불가(myapp은 **N/A** — PostgreSQL 미사용) | ID-07·08 |
...
| **보안 P-05~10** | 6(myapp: P-08~10 N/A, 실집계 3) | **P-08 은 즉시 중단**(myapp은 N/A) |
