...
> ⚠️ **myapp 적용 현황 (2026-08-29)** — myapp은 PostgreSQL을 쓰지 않습니다(`skipPaasData=true`). 아래 문서의 PostgreSQL 관련 런북(§3-x DB 연결 실패 대응, §4-x PITR 복원)은 이 플랫폼의 목표 절차이며 myapp에는 **N/A**입니다. myapp의 실제 정리 결과는 [`실습산출물/3차시/11_최종검토_대조.md`](../../실습산출물/3차시/11_최종검토_대조.md)를 참고하세요(2026-08-29 `90_cleanup.ps1`로 실제 정리 완료).
