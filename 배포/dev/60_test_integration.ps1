...
$results = @([ordered]@{
    Name = "통합 테스트 스위트 (test/integration → $base)"
    Status = $(if ($code -eq 0) { 'Pass' } else { 'Fail' })
    Detail = "npm run test:integration (exit=$code)"
})
$ok = New-TestReport -Env 'dev' -Kind 'integration' -Results $results -OutDir "$PSScriptRoot\reports"
if (-not $ok) { exit 1 }
